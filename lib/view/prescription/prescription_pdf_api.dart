...
import 'package:life_eazy/view/prescription/pdf_api.dart';
import 'package:pdf/pdf.dart';
import 'package:pdf/widgets.dart';
import 'package:printing/printing.dart';

import '../../models/prescription/get_prescription_response.dart';

class PdfParagraphApi {
  static Future<File> generate(
      GetPrescriptionResponse response, String image) async {
    final pdf = Document();
    final logo =
        (await rootBundle.load('images/logo2.png')).buffer.asUint8List();
    final signature =
        (await rootBundle.load('images/sign.png')).buffer.asUint8List();
    ...
    if (dob != null) {
      ...
      age = (currentTime.inDays / 365).floor().toString();
    }
    pdf.addPage(
      MultiPage(
          pageTheme: PageTheme(
            ...
            return Column(
              children: [
                Row(
                  ...
                  children: [
                    Text(
                        'Next FollowUp Date :"${formatDate(response.nextFollowUpDate.toString().split(' ').first)}',
                        style: TextStyle(
                ...
                Container(
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      "We aim to provide accessible, available & affordable online healthcare services to anyone anytime & anywhere log on to website  vivifyhealthcare.com  or reach us tollfree No 180034567654",
                      maxLines: 3,
    ...
    return PdfApi.saveDocument(
        name: '${response.userId?.firstname ?? "NA"}_Prescription.pdf',
        pdf: pdf);
  }

  static Widget doctorDetailsHeader(GetPrescriptionResponse response) =>
      Container(
        decoration: BoxDecoration(
          color: PdfColor.fromHex('32A887'),
        ),
  ...
  static Widget patientDetailsHeader(
          GetPrescriptionResponse response, String age) =>
      Container(
        child: Padding(
          padding: EdgeInsets.symmetric(horizontal: 12),
          child: Row(children: [
                ...
                children: [
                  Text(
                    response.familyMemberId == null
                        ? '${SessionManager.getUser.firstName ?? "NA"} ${SessionManager.getUser.lastName ?? "NA"}'
